...
5、控制对应的键盘操作：左划、右划、切换
"""

# 导入OpenCV
import cv2
# 导入handprocess
import virtual_mouse.handProcess
...
class VirtualMouse:
    ...
    def recognize(self):
        ...
        handprocess = virtual_mouse.handProcess.HandProcess(False,1)
        utils = Utils()
        ...
        while cap.isOpened():
            ...
            if key_point:
                ...
                if action_zh == '鼠标移动':
                    ...
                    pyautogui.scroll(-30)
                elif action_zh == '切换' and (now - action_trigger_time['switch'] > 2):
                    # taskCtl()
                    # time.sleep(1.0)
                    # pyautogui.press('tab')
                    # pyautogui.sleep(0.7)
                    pyautogui.hotkey('winleft', 'tab')
                    action_trigger_time['switch'] = now
                elif action_zh == '左划' and (now - action_trigger_time['Left stroke'] > 0.2):
                    pyautogui.press('left')
                    # pyautogui.sleep(0.1)
                    action_trigger_time['Left stroke'] = now
                elif action_zh == '右划' and (now - action_trigger_time['Right stroke'] > 0.2):
                    pyautogui.press('right')
                    # pyautogui.sleep(0.1)
                    action_trigger_time['Right stroke'] = now
            ...
            self.image = utils.cv2AddChineseText(self.image, '当前手势:'+str(action), (400, 0), textColor=(255, 0, 255), textSize=50)
