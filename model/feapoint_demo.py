...
mp_drawing = mp.solutions.drawing_utils  # 调库
mp_hands = mp.solutions.hands  #调库
input_points = [0, 2, 4, 5, 8, 9, 12, 13, 16, 17, 20]  # 手部11个关键点
...
two_hand = [7, 8, 9, 10, 11]  # 双手姿势
one_hand = [1, 2, 3, 4, 5, 6, 12, 13]  # 单手姿势 对应下面catigories
catigories = [
    ...
    "Zooming In",  # 8
    "Zooming Out",  # 9
    "Turning Clockwise",  # 10
    "Turning Counterclockwise",  # 11
    "Thumb Up",  # 12
    "Thumb Down",  # 13
]
...
class GestureRecognize:
    ...
    def gettt(self,idx):
        return self.__gesture_list[idx]  # ----------------------------------------------------------
    ...
    def main(self, show_windows=True):
        ...
        index = 0
        model = DNN()  # 神经网络
        model.load_state_dict(torch.load(model_path)) # 加载文件
        model.eval()
        ...
        print("Ready!")
        with mp_hands.Hands(
                min_detection_confidence=0.80,  # 手部检测置信度
                min_tracking_confidence=0.25) as hands:   # 手部跟踪置信度
            with torch.no_grad():   # 设置为无梯度
                while cap.isOpened():
                    t1 = time.time()   # 记录视觉
                    i_frame += 1
                    ...
                    image = cv2.cvtColor(cv2.flip(image, 1), cv2.COLOR_BGR2RGB)  # 将BGR改成RGB
                    # if i_frame%3 == 0:  # skip every other frame to obtain a suitable frame rate
                    # To improve performance, optionally mark the image as not writeable to
                    # pass by reference.
                    image.flags.writeable = False
                    results = hands.process(image)   # 放回结果集
                    ...
                    if True:
                        if results.multi_hand_landmarks:
                            hand1 = []  # 列表用于记录手部关键点的xyz坐标
                            hand2 = []  # 列表用于记录手部关键点的xyz坐标
                            hand_idx = 0   # 对应的手势序号
                            ...
                            for hand_landmarks in results.multi_hand_landmarks:
                                ...
                                if hand_idx == 1:
                                    for i in input_points:
                                        hand1.append(round(hand_landmarks.landmark[i].x, 3))  # 添加x
                                        hand1.append(round(hand_landmarks.landmark[i].y, 3))  # 添加y
                                        hand1.append(round(hand_landmarks.landmark[i].z, 3))  # 添加z
                                else:
                                    for i in input_points:
                        ...
                        # 阈值
                        if SOFTMAX_THRES > 0:
                            feat_np = out_put.numpy().reshape(-1)   # 转为narray类型
                            # print(feat_np)
                            feat_np -= feat_np.max()
                            softmax = np.exp(feat_np) / np.sum(np.exp(feat_np)) # 实现分类
                            # print(softmax)
                    ...
                    if show_windows:
                        t2 = time.time()   # 记录时间
                        current_time = t2 - t1  # 计算花费时间
                        image = cv2.resize(image, (640, 480))  # 改变图片大小
                        # image = image[:, ::-1]
                        height, width, _ = image.shape  # 获取图片的高、宽
                        ...
                        #
                        #cv2.setWindowProperty("python", cv2.WND_PROP_TOPMOST,1)
                        #os.system('''/usr/bin/osascript -e 'tell app "Finder" to set frontmost of process "python" to true' ''')
                        cv2.putText(label, '{:.1f} Vid/s'.format(1 / current_time),   # 计算帧率
                                    (width - 170, int(height / 16)),
                                    cv2.FONT_HERSHEY_SIMPLEX,
                                    0.7, (0, 0, 0), 2)
                        #
                        image = np.concatenate((image, label), axis=0)
                        cv2.imshow(WINDOW_NAME, image)  # TODO  # 显示画面
